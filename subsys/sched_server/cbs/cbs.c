...
#include <zephyr/sched_server/internal/cbs_internal.h>
...
static void cbs_budget_restore_if_condition(cbs_t *cbs)
{
	...
	/*
	 * The CBS condition is a way to see, when
     * a new job comes to the server, if
     * the current server bandwidth
     * (budget / deadline) is equal or lower
     * then the configured bandwidth. If that
     * is true, the job can be served with the
     * current budget and deadline. If not,
     * the deadline needs to be postponed.
     *
     * So basically, if the condition below
     * is met, we need to recalculate things.
     * We only check it when a new job comes:
     *
	 * Cs >= (ds - rjob) * U
     *
